...
async function renderOperationsHub(data){
  if(!data)return;const movements=data.movements||[],notifications=data.notifications||[],workflows=data.workflows||[],freightCorridors=data.freightCorridors||[],freightStationFacts=data.freightStationFacts||[],freightTracks=data.freightTracks||[];operationsMovements=new Map(movements.map(item=>[item.movement_id,item]));const invalidCoordinates=movements.filter(item=>item.latitude!=null&&item.longitude!=null&&!validUkraineOperationsPoint(Number(item.latitude),Number(item.longitude))).length;
