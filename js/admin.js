...
function renderRailIntelligence(data){
  ...
  nodes.railIntelligenceMetrics.append(metric("Переходы v3",counts.stateTransitions||0,"Append-only история фаз"),metric("Нет ожидаемого факта",counts.overdueStates||0,"Требует нового станционного события",counts.overdueStates?"warning":"ok"));
  ...
  const expected=data.expectedRegistry?.runs||[],gaps=data.expectedRegistry?.gaps||[],fusion=data.observationFusion?.groups||[],external=data.externalSources||[];
  nodes.railIntelligenceMetrics.append(metric("\u0421\u0443\u0442\u043e\u0447\u043d\u044b\u0439 \u0440\u0435\u0435\u0441\u0442\u0440",expected.length,`${expected.filter(item=>item.status==="planned").length} planned \u00b7 ${expected.filter(item=>item.status==="active").length} active`),metric("\u041c\u043e\u043b\u0447\u0430\u0449\u0438\u0435 \u0440\u0435\u0439\u0441\u044b",gaps.length,"\u041d\u0435 \u0438\u0441\u0447\u0435\u0437\u0430\u044e\u0442 \u0438\u0437 Operations Hub",gaps.length?"warning":"ok"),metric("Fusion v2",fusion.length,"\u0413\u0440\u0443\u043f\u043f\u044b \u0441\u0442\u0430\u043d\u0446\u0438\u043e\u043d\u043d\u044b\u0445 \u0444\u0430\u043a\u0442\u043e\u0432"),metric("\u041f\u043e\u0433\u0440\u0430\u043d\u0438\u0447\u043d\u044b\u0435 \u0438\u0441\u0442\u043e\u0447\u043d\u0438\u043a\u0438",external.filter(item=>item.status==="online").length,`${external.length} \u0430\u0434\u0430\u043f\u0442\u0435\u0440\u043e\u0432 \u00b7 \u0434\u043e\u0433\u043e\u0432\u043e\u0440\u043d\u044b\u0435 \u043f\u043e \u0443\u043c\u043e\u043b\u0447\u0430\u043d\u0438\u044e \u0437\u0430\u043a\u0440\u044b\u0442\u044b`,external.some(item=>item.status==="unavailable")?"warning":"neutral"));
...
async function renderOperationsMap(movements=[],freightCorridors=[],freightStationFacts=[]){
  const sequence=++operationsMapRenderSequence;ensureOperationsMap();if(!operationsLayer)return;const stationLookup=await operationsStationLookup();if(sequence!==operationsMapRenderSequence)return;operationsLayer.clearLayers();operationsMarkers=new Map();const bounds=[];
...
async function renderOperationsHub(data){
  if(!data)return;const movements=data.movements||[],notifications=data.notifications||[],workflows=data.workflows||[],freightCorridors=data.freightCorridors||[],freightStationFacts=data.freightStationFacts||[],freightTracks=data.freightTracks||[];operationsMovements=new Map(movements.map(item=>[item.movement_id,item]));const invalidCoordinates=movements.filter(item=>item.latitude!=null&&item.longitude!=null&&!validUkraineOperationsPoint(Number(item.latitude),Number(item.longitude))).length,reviewWorkflows=workflows.filter(item=>item.workflow_type==="twin_ambiguity");
  ...
  if(selectedMovementId&&!operationsMovements.has(selectedMovementId))selectedMovementId=null;renderOperationsSelection(selectedMovementId?operationsMovements.get(selectedMovementId):null);
