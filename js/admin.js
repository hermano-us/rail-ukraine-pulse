...
function renderRailIntelligence(data){
  ...
  const reference=data.graphReference; if(reference){const progress=reference.status==="active"?100:Math.round(((Number(reference.importedStations)||0)+(Number(reference.importedSegments)||0))/Math.max(1,(Number(reference.stations)||0)+(Number(reference.segments)||0))*100);nodes.railIntelligenceMetrics.append(metric("OSM Rail Graph",reference.status==="active"?reference.segments:`${progress}%`,reference.status==="active"?`${reference.activeDirectedSegments} направленных участков`:`Импорт ${reference.importedStations}/${reference.stations} станций`,reference.status==="active"?"ok":"warning"),metric("Реестр станций",reference.stations,`${reference.aliases} алиасов · ${reference.unmatchedStations} без пути`,reference.unmatchedStations>500?"warning":"ok"));}
