...
const nodes = {
  ...
  networkDisruptionRows: document.querySelector("#network-disruption-rows"),
...
const evidenceReviewInFlight=new Set();
...
async function renderEvidenceInbox(data={}){if(evidenceReviewInFlight.size)return;const sequence=++evidenceRenderSequence;let principal=null;try{const response=await fetch(authMeEndpoint,{cache:"no-store",headers:{Authorization:`Bearer ${token}`}});if(response.ok)principal=(await response.json()).user||null;}catch{}if(sequence!==evidenceRenderSequence||evidenceReviewInFlight.size)return;const evidence=data.evidence||[],canReview=canReviewEvidence(principal);nodes.freightEvidenceCount.textContent=`${evidence.length} в очереди`;nodes.freightEvidenceRows.replaceChildren();freightReviewStatus(canReview?"":"Решения доступны администратору или старшему куратору.");if(!evidence.length){const row=document.createElement("tr"),cell=appendCell(row,"Очередь свидетельств пуста","empty-row");cell.colSpan=8;nodes.freightEvidenceRows.append(row);return;}for(const item of evidence){const row=document.createElement("tr");row.dataset.evidenceId=item.evidence_id;appendCell(row,formatDate(item.occurred_at));appendCell(row,item.source_id);appendCell(row,item.corridor_code);let classification={};try{classification=JSON.parse(item.classification_json||"{}");}catch{}appendCell(row,classification.freightType||item.domain);appendCell(row,`${Math.round(Number(item.confidence||0)*100)}%`);const excerpt=appendCell(row,item.evidence_excerpt);excerpt.title=item.source_url||"";appendCell(row,item.review_status,`status-pill ${item.review_status}`);const actions=document.createElement("td");actions.className="evidence-actions";if(canReview){for(const [status,label] of [["corroborated","Подтвердить"],["needs_context","Уточнить"],["rejected","Отклонить"]]){const button=document.createElement("button");button.type="button";button.className="secondary";button.textContent=label;button.addEventListener("click",()=>reviewEvidence(item.evidence_id,status,row));actions.append(button);}}else actions.textContent="Только просмотр";row.append(actions);nodes.freightEvidenceRows.append(row);}}
async function reviewEvidence(evidenceId,status,row){if(evidenceReviewInFlight.has(evidenceId))return;evidenceReviewInFlight.add(evidenceId);const buttons=[...(row?.querySelectorAll("button")||[])];buttons.forEach(button=>button.disabled=true);row?.setAttribute("aria-busy","true");freightReviewStatus("Сохраняем решение…");try{const response=await fetch(`${evidenceEndpoint}/review`,{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify({evidenceId,status})});if(!response.ok){const body=await response.json().catch(()=>({}));throw new Error(response.status===403?"Недостаточно прав для разбора грузовых сигналов":body.error||`Evidence review HTTP ${response.status}`);}evidenceReviewInFlight.delete(evidenceId);const queue=await fetch(evidenceEndpoint,{cache:"no-store",headers:{Authorization:`Bearer ${token}`}});if(queue.ok)await renderEvidenceInbox(await queue.json());else{row?.remove();nodes.freightEvidenceCount.textContent=`${nodes.freightEvidenceRows.querySelectorAll("tr[data-evidence-id]").length} в очереди`;}freightReviewStatus("Решение сохранено","ok");}catch(error){evidenceReviewInFlight.delete(evidenceId);row?.removeAttribute("aria-busy");buttons.forEach(button=>button.disabled=false);freightReviewStatus(error.message,"error");}}
...
function renderRailIntelligence(data){
  ...
  const expected=data.expectedRegistry?.runs||[],gaps=data.expectedRegistry?.gaps||[],graphGaps=data.graph?.gaps||[],fusion=data.observationFusion?.groups||[],external=data.externalSources||[],priorities=data.stationCoveragePriorities||[],collectionPlan=data.stationCollectionPlan||[],modelGovernance=data.modelGovernance||{},activeModel=modelGovernance.activeModel||"rail-intelligence-v5",routeRouting=data.routeRouting||{};
  const readiness=data.operationalReadiness||{};
  nodes.railIntelligenceMetrics.append(metric("\u0413\u043e\u0442\u043e\u0432\u043d\u043e\u0441\u0442\u044c \u043a\u043e\u043d\u0442\u0443\u0440\u0430",String(readiness.readyCount||0)+"/"+String(readiness.totalChecks||4),readiness.status||"insufficient-evidence",readiness.status==="operational"?"ok":readiness.status==="warming"?"neutral":"warning"));
  const reliability=data.dataReliability||{},collectors=reliability.collectors||[],activeCollectors=collectors.filter(item=>["healthy","starting"].includes(item.status)).length,criticalStations=priorities.filter(item=>item.priority_tier==="critical").length;
  nodes.railIntelligenceMetrics.append(metric("Data Reliability v2",`${activeCollectors}/${collectors.length} collectors`,`${criticalStations} \u043a\u0440\u0438\u0442\u0438\u0447\u0435\u0441\u043a\u0438\u0445 \u0441\u0442\u0430\u043d\u0446\u0438\u0439`,collectors.length&&!activeCollectors?"warning":"ok"));
  nodes.railIntelligenceMetrics.append(metric("\u0421\u0443\u0442\u043e\u0447\u043d\u044b\u0439 \u0440\u0435\u0435\u0441\u0442\u0440",expected.length,`${expected.filter(item=>item.status==="planned").length} planned \u00b7 ${expected.filter(item=>item.status==="active").length} active`),metric("\u041c\u043e\u043b\u0447\u0430\u0449\u0438\u0435 \u0440\u0435\u0439\u0441\u044b",gaps.length,"\u041d\u0435 \u0438\u0441\u0447\u0435\u0437\u0430\u044e\u0442 \u0438\u0437 Operations Hub",gaps.length?"warning":"ok"),metric("Fusion v5",fusion.length,`${fusion.filter(item=>item.status==="conflict").length} \u043a\u043e\u043d\u0444\u043b\u0438\u043a\u0442\u043e\u0432`,fusion.some(item=>item.status==="conflict")?"warning":"ok"),metric("\u041f\u0440\u0438\u043e\u0440\u0438\u0442\u0435\u0442\u044b \u0442\u0430\u0431\u043b\u043e",priorities.length,`${priorities.filter(item=>Number(item.priority_score)>=40).length} \u0441\u0440\u043e\u0447\u043d\u044b\u0445 \u0441\u0442\u0430\u043d\u0446\u0438\u0439`),metric("\u041f\u043e\u0433\u0440\u0430\u043d\u0438\u0447\u043d\u044b\u0435 \u0438\u0441\u0442\u043e\u0447\u043d\u0438\u043a\u0438",external.filter(item=>item.status==="online").length,`${external.length} \u0430\u0434\u0430\u043f\u0442\u0435\u0440\u043e\u0432 \u00b7 \u0434\u043e\u0433\u043e\u0432\u043e\u0440\u043d\u044b\u0435 \u043f\u043e \u0443\u043c\u043e\u043b\u0447\u0430\u043d\u0438\u044e \u0437\u0430\u043a\u0440\u044b\u0442\u044b`,external.some(item=>item.status==="unavailable")?"warning":"neutral"));
  nodes.railIntelligenceMetrics.append(metric("Разрывы Rail Graph",graphGaps.length,"Автоматически найденные границы компонентов",graphGaps.length?"warning":"ok"),metric("Route-aware маршруты",routeRouting.routes||0,routeRouting.averageConfidence==null?"Кэш прогревается":`${Math.round(routeRouting.averageConfidence*100)}% уверенность · ${routeRouting.routesWithAlternatives||0} с альтернативами`,routeRouting.routes?"ok":"warning"),metric("Адаптивный опрос",collectionPlan.filter(item=>Number(item.target_interval_minutes)<=5).length,`${collectionPlan.length} станций · критические каждые 2 мин`),metric("Активная модель",activeModel.replace("rail-intelligence-","v"),`${modelGovernance.qualityWindows?.length||0} контрольных окон`,modelGovernance.releases?.some(item=>item.status==="rolled_back")?"warning":"ok"));
...
function sliceLineProgress(points,start=0,end=1){if(!Array.isArray(points)||points.length<2)return points;const cumulative=[0];for(let index=1;index<points.length;index+=1)cumulative.push(cumulative.at(-1)+Math.hypot(points[index][0]-points[index-1][0],points[index][1]-points[index-1][1]));const total=cumulative.at(-1)||1,target=(progress)=>Math.max(0,Math.min(1,Number(progress)||0))*total,pointAt=(distance)=>{let index=1;while(index<cumulative.length&&cumulative[index]<distance)index+=1;if(index>=points.length)return points.at(-1);const span=Math.max(Number.EPSILON,cumulative[index]-cumulative[index-1]),ratio=(distance-cumulative[index-1])/span;return [points[index-1][0]+(points[index][0]-points[index-1][0])*ratio,points[index-1][1]+(points[index][1]-points[index-1][1])*ratio];},from=target(start),to=Math.max(from,target(end));return [pointAt(from),...points.filter((_,index)=>cumulative[index]>from&&cumulative[index]<to),pointAt(to)];}
function hypothesisCoordinates(hypothesis){const geometry=hypothesis?.geometry,coordinates=geometry?.type==="LineString"?geometry.coordinates:Array.isArray(geometry)?geometry:null;if(!Array.isArray(coordinates)||coordinates.length<2)return null;const interval=hypothesis.reasons?.progressInterval,band=interval?sliceLineProgress(coordinates,interval.p10,interval.p90):coordinates,latLngs=band.map(point=>[Number(point?.[1]),Number(point?.[0])]);return latLngs.every(([lat,lon])=>validUkraineOperationsPoint(lat,lon))?latLngs:null;}
function renderSelectedTwinLayer(item){if(!operationsTwinLayer)return;operationsTwinLayer.clearLayers();if(!item)return;const history=(item.probabilityHistory||[]).map(sample=>[Number(sample.latitude),Number(sample.longitude)]).filter(([lat,lon])=>validUkraineOperationsPoint(lat,lon));if(history.length>1){const trail=L.polyline(history,{color:"#ffd166",weight:3,opacity:.65,dashArray:"2 7"}).addTo(operationsTwinLayer);trail.bindTooltip(`Изменение вероятного положения за 24 часа · ${history.length} снимков`);for(const [index,point] of history.entries())if(index===0||index===history.length-1||index%6===0)L.circleMarker(point,{radius:index===history.length-1?5:3,color:"#ffd166",weight:1,fillOpacity:.45,interactive:false}).addTo(operationsTwinLayer);}for(const [index,hypothesis] of (item.twinHypotheses||[]).slice(0,3).entries()){const latLngs=hypothesisCoordinates(hypothesis);if(!latLngs||latLngs.length<2)continue;const probability=Number(hypothesis.probability||0),uncertainty=Number(hypothesis.uncertainty_km||0),primary=index===0,interval=hypothesis.reasons?.progressInterval;L.polyline(latLngs,{color:primary?"#5ce1c5":"#8da8ff",weight:Math.max(12,Math.min(30,uncertainty/3)),opacity:.08,interactive:false,lineCap:"round"}).addTo(operationsTwinLayer);const line=L.polyline(latLngs,{color:primary?"#5ce1c5":"#8da8ff",weight:primary?4:3,opacity:Math.max(.35,probability),dashArray:primary?"10 7":"5 8"}).addTo(operationsTwinLayer);line.bindTooltip(`${primary?"Основной диапазон":"Альтернатива"}: ${hypothesis.from_node_id} → ${hypothesis.to_node_id} · ${Math.round(probability*100)}%${interval?` · P10–P90 ${Math.round(interval.p10*100)}–${Math.round(interval.p90*100)}%`:""}`);}}
...
async function renderOperationsMap(movements=[],freightCorridors=[],freightStationFacts=[],networkDisruptions=[]){
  ...
  for(const disruption of networkDisruptions){const geometry=disruption.geometry,coordinates=geometry?.type==="LineString"?geometry.coordinates:null;if(!Array.isArray(coordinates)||coordinates.length<2)continue;const latLngs=coordinates.map(([lon,lat])=>[lat,lon]).filter(([lat,lon])=>validUkraineOperationsPoint(Number(lat),Number(lon)));if(latLngs.length<2)continue;const line=L.polyline(latLngs,{color:"#ff5968",weight:7,opacity:.75,dashArray:"4 7"}).addTo(operationsLayer);line.bindTooltip(`Нарушение движения · ${disruption.evidence?.affectedRuns?.length||0} поездов · ${Math.round(Number(disruption.score||0)*100)}%`);for(const point of latLngs)bounds.push(point);}
...
async function renderOperationsHub(data){
  ...
  if(nodes.networkDisruptionRows){if(!networkDisruptions.length)emptyTable(nodes.networkDisruptionRows,"Активных сетевых нарушений не обнаружено.",6);else{nodes.networkDisruptionRows.replaceChildren();for(const item of networkDisruptions){const row=document.createElement("tr"),evidence=item.evidence||{};appendCell(row,`${evidence.fromNodeId||item.node_id||"?"} → ${evidence.toNodeId||"?"}`);appendCell(row,evidence.classification||"possible",`status-pill ${item.severity}`);appendCell(row,(evidence.affectedRuns||[]).length);appendCell(row,evidence.maximumDelayMinutes?`${Math.round(evidence.maximumDelayMinutes)} мин`:"—");appendCell(row,`${Math.round(Number(item.score||0)*100)}%`);appendCell(row,formatAge(item.detected_at));nodes.networkDisruptionRows.append(row);}}}
...
function renderAnalyticsNetwork(data){if(!data)return;const activity=data.nodeActivity||[],anomalies=data.anomalies||[],corridors=data.corridors||[],calibration=data.calibration||{},qualityGate=data.qualityGate||{},governance=data.modelGovernance||{},activeRelease=(governance.releases||[]).find(item=>item.status==="active"),cycles=data.cycles||[],profiles=data.calibrationProfiles||[],models=data.calibrationByModel||[];nodes.analyticsNetworkMetrics.replaceChildren(metric("Активные узлы",activity.length,"Node Activity Score"),metric("Аномалии",anomalies.length,"Открытые сигналы",anomalies.length?"warning":"ok"),metric("MAE",calibration.maeMinutes==null?"—":`${calibration.maeMinutes} мин`,`${calibration.prospectiveEvaluations||0} live · ${calibration.replayEvaluations||0} replay`),metric("P80",calibration.p80Coverage==null?"—":`${calibration.p80Coverage}%`,calibration.readiness||"insufficient-evidence"),metric("Модель production",activeRelease?.model_version?.replace("rail-intelligence-","v")||"—",`${governance.qualityWindows?.length||0} quality windows`,governance.releases?.some(item=>item.status==="rolled_back")?"warning":"ok"),metric("Автономные циклы",cycles.filter(item=>item.status==="success").length,"Успешные запуски"));
  ...
  const v5Quality=models.find(item=>String(item.model_version||"").includes("v5"));
  nodes.analyticsNetworkMetrics.append(metric("MODEL v5",v5Quality?.evaluations||0,v5Quality?("MAE "+String(v5Quality.maeMinutes??"—")+" · P80 "+String(v5Quality.p80Coverage??"—")+"%"):"Накопление перспективных проверок",v5Quality?.evaluations>=20?"ok":"neutral"));
  const latestCycle=cycles[0]||{};
  nodes.analyticsNetworkMetrics.append(metric("Fusion v5",latestCycle.fused_observations||0,`${latestCycle.fusion_ambiguous||0} \u043a\u043e\u043d\u0444\u043b\u0438\u043a\u0442\u043e\u0432`,Number(latestCycle.fusion_ambiguous)?"warning":"ok"),metric("Twin v5",latestCycle.twins_recalculated||0,"\u041f\u0435\u0440\u0435\u0441\u0447\u0438\u0442\u0430\u043d\u043e \u043f\u043e \u043d\u043e\u0432\u044b\u043c \u0444\u0430\u043a\u0442\u0430\u043c"),metric("\u041f\u043e\u043a\u0440\u044b\u0442\u0438\u0435 \u0442\u0430\u0431\u043b\u043e",latestCycle.board_priorities||0,"\u0410\u0434\u0430\u043f\u0442\u0438\u0432\u043d\u044b\u0435 \u043f\u0440\u0438\u043e\u0440\u0438\u0442\u0435\u0442\u044b \u0441\u0442\u0430\u043d\u0446\u0438\u0439"));
