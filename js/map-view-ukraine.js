...
export class MapView{
  constructor(elementId,onSelect){
    ...
    this.routeLayer=L.geoJSON(null,{interactive:false,style:(feature)=>{const count=this.routeIntensity?.get(feature?.properties?.id)||1,provisional=Boolean(feature?.properties?.probabilisticFallback),constrained=feature?.properties?.routeVerification?.status==="constrained";return {color:provisional?"#a87645":constrained?"#d59a58":count>4?"#f3b562":"#2f8a9d",weight:provisional?1.2:constrained?1.5:1.1+Math.min(4,Math.log2(count+1)),opacity:provisional?.24:constrained?.42:.12+Math.min(.5,count*.055),dashArray:provisional?"3 9":constrained?"8 6":null};}}).addTo(this.map);
