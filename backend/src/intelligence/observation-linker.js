...
export function scoreRunCandidateDetails(event, candidate) {
  ...
  const station=normalize(event.station),candidateRoute=normalize([candidate.origin,candidate.route,candidate.destination].filter(Boolean).join(" "));
  const stationMatch=Boolean(station&&candidateRoute.includes(station));if(stationMatch)score+=.08;features.push(feature("station",stationMatch?"Станция присутствует в маршруте":"Станция не подтверждена маршрутом",stationMatch?.08:-.03,stationMatch,event.station||null));if(station&&!stationMatch)score-=.03;
