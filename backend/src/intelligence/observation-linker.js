...
export function scoreRunCandidateDetails(event, candidate) {
  const observedTrain=trainKey(event?.train_number),candidateTrain=trainKey(candidate.train_number);
  if(observedTrain&&observedTrain!==candidateTrain)return null;
  ...
  const numberWeight=observedTrain?.42:0,features=[feature("train_number",observedTrain?"\u0421\u043e\u0432\u043f\u0430\u0434\u0430\u0435\u0442 \u043d\u043e\u043c\u0435\u0440 \u043f\u043e\u0435\u0437\u0434\u0430":"\u041d\u043e\u043c\u0435\u0440 \u043d\u0435 \u0438\u0437\u0432\u043b\u0435\u0447\u0451\u043d",numberWeight,Boolean(observedTrain),String(candidate.train_number||""))]; let score=numberWeight;
  ...
  const fusionGrade=event.evidence_grade||event.evidenceGrade,independentSources=Number(event.independent_domains||event.independent_sources)||0,fusionReliability=Number(event.fusion_reliability);
  ...
  const negativeEvidence=Array.isArray(event.negative_evidence)?event.negative_evidence:[];
  const negativeMatch=negativeEvidence.some((value)=>{const key=normalize(value);return key&&(key===normalize(candidate.run_id)||key===candidateTrain);});
  if(negativeMatch){score-=.28;features.push(feature("negative_evidence","\u041a\u0430\u043d\u0434\u0438\u0434\u0430\u0442 \u0438\u0441\u043a\u043b\u044e\u0447\u0451\u043d \u043e\u0442\u0440\u0438\u0446\u0430\u0442\u0435\u043b\u044c\u043d\u044b\u043c \u0444\u0430\u043a\u0442\u043e\u043c",-.28,false));}
...
export function chooseCanonicalRun(event, candidates = []) {
  ...
  const corroborated=event.evidence_grade==="corroborated"&&Number(event.independent_domains||event.independent_sources)>=2;
...
export async function linkRecentObservations(env, now = new Date().toISOString(), stationAliases = new Map()) {
  const [eventResult,runResult,expectedResult]=await Promise.all([
    env.DB.prepare(`SELECT e.event_id,e.run_id original_run_id,e.station,e.occurred_at,e.raw_update_json,o.train_number,o.service_date,o.route,o.origin,o.destination,fg.independent_sources,fg.source_domains independent_domains,fg.effective_reliability fusion_reliability,fg.negative_evidence_json,fg.explanation_json fusion_explanation_json
  ...
  for(const event of rows(eventResult)){
    const raw=parseJson(event.raw_update_json),fusion=parseJson(event.fusion_explanation_json),stationKey=normalize(event.station),enriched={...event,evidence_grade:fusion.evidenceGrade||null,negative_evidence:Array.isArray(fusion.negativeEvidence)?fusion.negativeEvidence:[],train_number:event.train_number||raw.trainNumber||raw.train_number,service_date:event.service_date||raw.serviceDate,route:event.route||raw.route,origin:event.origin||raw.origin,destination:event.destination||raw.destination,locomotive:raw.locomotive||raw.locomotiveNumber||null,station_id:stationAliases.get(stationKey)||null};
    const matching=enriched.train_number?runs.filter((run)=>trainKey(run.train_number)===trainKey(enriched.train_number)):runs,decision=chooseCanonicalRun(enriched,matching);if(decision.status==="linked")linked+=1;else pending+=1;
    ...
    if(decision.status==="pending")statements.push(env.DB.prepare(`INSERT INTO ops_workflows(workflow_id,movement_id,workflow_type,state,priority,title,description,created_by,created_at,updated_at) VALUES(?1,?2,'observation_link_review','open',?3,?4,?5,'entity-resolution-v4',?6,?6) ON CONFLICT(workflow_id) DO UPDATE SET state=CASE WHEN ops_workflows.state='resolved' THEN ops_workflows.state ELSE 'open' END,priority=excluded.priority,description=excluded.description,updated_at=excluded.updated_at`).bind(workflowId,event.original_run_id||event.event_id,decision.reviewReason==="candidate-conflict"?"high":"normal",`\u0423\u0442\u043e\u0447\u043d\u0438\u0442\u044c \u043f\u0440\u0438\u0432\u044f\u0437\u043a\u0443 \u043d\u0430\u0431\u043b\u044e\u0434\u0435\u043d\u0438\u044f ${enriched.train_number||event.event_id}`,`${decision.candidates.length} \u043a\u0430\u043d\u0434\u0438\u0434\u0430\u0442\u043e\u0432; \u043b\u0443\u0447\u0448\u0438\u0439 ${Math.round(decision.confidence*100)}%, \u0440\u0430\u0437\u0440\u044b\u0432 ${Math.round(decision.margin*100)} \u043f.\u043f.`,now));
    else statements.push(env.DB.prepare("UPDATE ops_workflows SET state='resolved',resolved_at=?1,resolution='linked automatically by entity-resolution-v4',updated_at=?1 WHERE workflow_id=?2 AND state!='resolved'").bind(now,workflowId));
  }
