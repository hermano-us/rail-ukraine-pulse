...
function contextHash(pair){const serialized=JSON.stringify(normalizedContext(pair));let hash=2166136261;for(let index=0;index<serialized.length;index+=1){hash^=serialized.charCodeAt(index);hash=Math.imul(hash,16777619);}return `v7-${(hash>>>0).toString(16).padStart(8,"0")}`;}
...
export async function resolveRailRouteGeometries(env, pairs = [], now = new Date().toISOString(), { maximumNewRoutes=12, retryNoPathAfterMinutes=360 } = {}) {
  ...
  const missing=unique.filter((pair)=>!cachedContexts.has(`${routeKey(pair)}:${pair.contextHash}`)).slice(0,Math.max(0,maximumNewRoutes));if(!missing.length)return {versionId:version.version_id,routes,calculated:0};
