...
function transitionPenalty(state,leg){if(!state.nodes.length)return 0;const previousNode=state.nodes.at(-2),nextNode=leg.nodes[1];if(previousNode&&previousNode===nextNode)return Infinity;const priorNodes=new Set(state.nodes.slice(0,-1)),repeated=leg.nodes.slice(1).filter((node)=>priorNodes.has(node)).length;if(repeated)return 500+repeated*100;const priorRelations=relationSet(state.edges.slice(-4)),nextRelations=relationSet(leg.edges.slice(0,4)),shared=[...priorRelations].some((id)=>nextRelations.has(id));if(priorRelations.size&&nextRelations.size&&!shared)return Math.max(3,leg.routingCost*.25);return shared?-Math.min(7,leg.routingCost*.08):0;}
export function validateCompiledRoute(path,requiredStops=[],context={}){const indices=[];let cursor=0;for(const stop of requiredStops){const index=path.nodes.indexOf(stop,cursor);if(index<0)return {valid:false,reason:"mandatory_station_missing",matchedStops:indices.length,requiredStops:requiredStops.length};indices.push(index);cursor=index+1;}const reversals=path.nodes.slice(2).filter((node,index)=>node===path.nodes[index]).length,serviceShare=path.edges.reduce((sum,edge)=>sum+(edge.services?.length?(edge.serviceShare==null?1:Number(edge.serviceShare)):0),0)/Math.max(1,path.edges.length);if(reversals&&!context.allowReversal)return {valid:false,reason:"unexpected_reversal",matchedStops:indices.length,requiredStops:requiredStops.length,reversals,serviceShare};if(serviceShare>.4&&!context.allowServiceTracks)return {valid:false,reason:"excessive_service_track_share",matchedStops:indices.length,requiredStops:requiredStops.length,reversals,serviceShare};return {valid:true,reason:"mandatory_stations_in_order",matchedStops:indices.length,requiredStops:requiredStops.length,reversals,serviceShare:Number(serviceShare.toFixed(3))};}
