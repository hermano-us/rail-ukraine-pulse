...
function legAlternatives(adjacency,from,to,context,options={}){
  const maximum=Math.max(1,Math.min(5,Number(options.maximumLegCandidates)||(context.waypoints?.length>8?2:3))),primary=weightedPath(adjacency,from,to,context,options);if(!primary)return [];
  const candidates=new Map([[primary.nodes.join(">"),primary]]),signatures=primary.nodes.slice(0,-1).map((node,index)=>`${node}>${primary.nodes[index+1]}`).slice(0,Math.max(10,maximum*5));
  for(const signature of signatures){const alternative=weightedPath(adjacency,from,to,context,{...options,bannedEdges:new Set([...(options.bannedEdges||[]),signature])});if(alternative)candidates.set(alternative.nodes.join(">"),alternative);if(candidates.size>=maximum*3)break;}
  return [...candidates.values()].sort((left,right)=>left.routingCost-right.routingCost).slice(0,maximum);
}
function relationSet(edges=[]){return new Set(edges.flatMap((edge)=>edge.routeRelationIds||[]).map(String));}
function stationThroatTurn(previousEdge,nextEdge){const services=new Set([...(previousEdge?.services||[]),...(nextEdge?.services||[])]),distance=Math.max(Number(previousEdge?.distance)||Infinity,Number(nextEdge?.distance)||Infinity),serviceShare=Math.max(Number(previousEdge?.serviceShare)||0,Number(nextEdge?.serviceShare)||0);return distance<=5&&serviceShare<=.5&&["siding","crossover"].some((service)=>services.has(service));}
function transitionPenalty(state,leg){if(!state.nodes.length)return 0;const previousNode=state.nodes.at(-2),nextNode=leg.nodes[1];if(previousNode&&previousNode===nextNode){const previousEdge=state.edges.at(-1),nextEdge=leg.edges[0];return stationThroatTurn(previousEdge,nextEdge)?30+Math.max(Number(previousEdge?.distance)||0,Number(nextEdge?.distance)||0)*3:Infinity;}const priorNodes=new Set(state.nodes.slice(0,-1)),repeated=leg.nodes.slice(1).filter((node)=>priorNodes.has(node)).length;if(repeated)return 500+repeated*100;const priorRelations=relationSet(state.edges.slice(-4)),nextRelations=relationSet(leg.edges.slice(0,4)),shared=[...priorRelations].some((id)=>nextRelations.has(id));if(priorRelations.size&&nextRelations.size&&!shared)return Math.max(3,leg.routingCost*.25);return shared?-Math.min(7,leg.routingCost*.08):0;}
export function validateCompiledRoute(path,requiredStops=[],context={}){const indices=[];let cursor=0;for(const stop of requiredStops){const index=path.nodes.indexOf(stop,cursor);if(index<0)return {valid:false,reason:"mandatory_station_missing",matchedStops:indices.length,requiredStops:requiredStops.length};indices.push(index);cursor=index+1;}const reversalIndexes=path.nodes.slice(2).map((node,index)=>node===path.nodes[index]?index:null).filter((index)=>index!=null),stationThroatReversals=reversalIndexes.filter((index)=>requiredStops.includes(path.nodes[index+1])&&stationThroatTurn(path.edges[index],path.edges[index+1])).length,reversals=reversalIndexes.length,unexpectedReversals=reversals-stationThroatReversals,serviceShare=path.edges.reduce((sum,edge)=>sum+(edge.services?.length?(edge.serviceShare==null?1:Number(edge.serviceShare)):0),0)/Math.max(1,path.edges.length);if(unexpectedReversals&&!context.allowReversal)return {valid:false,reason:"unexpected_reversal",matchedStops:indices.length,requiredStops:requiredStops.length,reversals,stationThroatReversals,serviceShare};if(serviceShare>.4&&!context.allowServiceTracks)return {valid:false,reason:"excessive_service_track_share",matchedStops:indices.length,requiredStops:requiredStops.length,reversals,stationThroatReversals,serviceShare};return {valid:true,reason:"mandatory_stations_in_order",matchedStops:indices.length,requiredStops:requiredStops.length,reversals,stationThroatReversals,serviceShare:Number(serviceShare.toFixed(3))};}

export function compileItineraryCandidates(adjacency,from,to,context={},options={}){
  const stops=[from,...new Set((context.waypoints||[]).filter((item)=>item&&item!==from&&item!==to)),to],maximumCandidates=Math.max(1,Math.min(5,Number(options.maximumCandidates)||3)),beamWidth=Math.max(6,maximumCandidates*4);let beam=[{nodes:[],edges:[],distanceKm:0,routingCost:0,continuityPenalty:0,legChoices:[]}];
  for(let index=1;index<stops.length;index+=1){const legs=legAlternatives(adjacency,stops[index-1],stops[index],context,options);if(!legs.length)return [];const expanded=[];for(const state of beam)for(let candidateIndex=0;candidateIndex<legs.length;candidateIndex+=1){const leg=legs[candidateIndex],penalty=transitionPenalty(state,leg);if(!Number.isFinite(penalty))continue;expanded.push({nodes:[...state.nodes,...(state.nodes.length?leg.nodes.slice(1):leg.nodes)],edges:[...state.edges,...leg.edges],distanceKm:state.distanceKm+leg.distanceKm,routingCost:state.routingCost+leg.routingCost,continuityPenalty:state.continuityPenalty+penalty,legChoices:[...state.legChoices,candidateIndex]});}beam=[...new Map(expanded.sort((left,right)=>(left.routingCost+left.continuityPenalty)-(right.routingCost+right.continuityPenalty)).map((item)=>[item.nodes.join(">"),item])).values()].slice(0,beamWidth);if(!beam.length)return [];}
  const valid=beam.map((path)=>({...path,distanceKm:Number(path.distanceKm.toFixed(3)),routingCost:Number(path.routingCost.toFixed(3)),compilationCost:Number((path.routingCost+path.continuityPenalty).toFixed(3)),hopCount:path.nodes.length-1,validation:validateCompiledRoute(path,stops,context)})).filter((path)=>path.validation.valid).sort((left,right)=>left.compilationCost-right.compilationCost).slice(0,maximumCandidates);if(!valid.length)return [];
  const best=valid[0].compilationCost,second=valid[1]?.compilationCost??best*1.35,ambiguity=Math.max(0,Math.min(1,(second-best)/Math.max(1,best)));
  return valid.map((path,index)=>{const relationEdges=path.edges.filter((edge)=>edge.routeRelationIds.length).length,serviceEdges=path.edges.filter((edge)=>edge.services.length).length,relative=best/Math.max(best,path.compilationCost),confidence=Math.max(.38,Math.min(.99,.6+.14*relative+.1*(relationEdges/Math.max(1,path.edges.length))+.1*Math.min(1,(stops.length-2)/5)+.08*ambiguity-.12*(serviceEdges/Math.max(1,path.edges.length))));return {...path,rank:index+1,score:Number((relative*100).toFixed(2)),confidence:Number(confidence.toFixed(3)),explanation:{method:"itinerary-constrained-v1",compilerVersion:"rail-route-compiler-v1",requiredWaypoints:context.waypoints||[],itineraryHash:context.itineraryHash||null,serviceDate:context.serviceDate||null,legChoices:path.legChoices,continuityPenalty:Number(path.continuityPenalty.toFixed(3)),matchedRouteRelationEdges:relationEdges,serviceTrackEdges:serviceEdges,physicalDistanceKm:path.distanceKm,routingCost:path.routingCost,validation:path.validation}};});
}

export function routeAwareCandidates(adjacency,from,to,context={},options={}){
  return compileItineraryCandidates(adjacency,from,to,context,options);
}
