...
async function analyticsNetwork(request,env,principal,json){
  if(!allowed(principal,"analytics.network.read","admin.overview"))return json({error:"forbidden"},403);if(request.method!=="GET")return json({error:"method_not_allowed"},405);const [activity,anomalies,corridors,cycles,quality]=await Promise.all([
    ...
    env.DB.prepare(`SELECT COUNT(*) evaluations,AVG(absolute_error_minutes) mae_minutes,AVG(within_p80)*100 p80_coverage,SUM(CASE WHEN evaluation_id LIKE 'replay:%' THEN 1 ELSE 0 END) replay_evaluations,SUM(CASE WHEN evaluation_id NOT LIKE 'replay:%' THEN 1 ELSE 0 END) prospective_evaluations,MAX(evaluated_at) latest FROM model_evaluations WHERE evaluated_at>=datetime('now','-30 days')`).first(),
  ]);return json({generatedAt:new Date().toISOString(),nodeActivity:rows(activity),anomalies:rows(anomalies),corridors:rows(corridors).map(item=>({...item,countries:JSON.parse(item.countries_json||"[]"),borderNodes:JSON.parse(item.border_nodes_json||"[]")})),cycles:rows(cycles),calibration:{evaluations:Number(quality?.evaluations||0),replayEvaluations:Number(quality?.replay_evaluations||0),prospectiveEvaluations:Number(quality?.prospective_evaluations||0),readiness:Number(quality?.prospective_evaluations||0)>=20?"operational":Number(quality?.evaluations||0)>=10?"warming":"insufficient-evidence",maeMinutes:quality?.mae_minutes==null?null:Number(Number(quality.mae_minutes).toFixed(1)),p80Coverage:quality?.p80_coverage==null?null:Number(Number(quality.p80_coverage).toFixed(1)),latest:quality?.latest||null}});
}
