...
export function fuseObservationRowsV3(events = [], {
  windowMinutes = 20,
  ...
  return groups.map((group) => {
    ...
    const sourceIds = [...sourceBest.keys()];
    const effectiveReliability = 1 - [...sourceBest.values()].reduce((product, value) => product * (1 - Math.min(.96, value)), 1);
    ...
    const evidenceGrade = conflicts.length ? "conflict"
      : sourceIds.length >= 2 && effectiveReliability >= .8 ? "corroborated"
    ...
    return {
      ...group,
      fusionId: `fusion-v3:${trainKey(group.trainNumber)}:${group.stationId}:${bucket}`,
      ...
      explanation: {
        method: "observation-fusion-v3", memberCount: group.members.length,
        independentSources: sourceIds.length, canonicalStationId: group.stationId,
        canonicalOccurredAt, evidenceGrade, conflicts, directions, locomotives,
        windowMinutes,
      },
    };
  });
...
export async function fuseRecentObservationsV3(env, now = new Date().toISOString(), stationAliases = new Map()) {
  ...
  const groups = fuseObservationRowsV3(events, { canonicalStation });
  const statements = [];
  for (const group of groups) {
    statements.push(env.DB.prepare(`INSERT INTO observation_fusion_groups(
      ...
      member_count,independent_sources,effective_reliability,status,source_ids_json,explanation_json,updated_at)
      ...
      .bind(group.fusionId, group.trainNumber, group.stationId, group.windowStart,
        ...
        JSON.stringify(group.sourceIds), JSON.stringify(group.explanation), now));
    for (const member of group.members) {
