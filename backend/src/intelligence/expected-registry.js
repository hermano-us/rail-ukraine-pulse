...
export function classifyExpectedRun(run, now = new Date().toISOString()) {
  ...
  if (last && current - last <= 90 * 60_000) return { status: "active", reason: "fresh-observation", missingSince: null };
  if (last && current - last <= 360 * 60_000) return { status: "unobserved", reason: "observation-gap", missingSince: new Date(last + 90 * 60_000).toISOString() };
  if (departure && current < departure + 20 * 60_000) return { status: "planned", reason: "departure-window-not-open", missingSince: null };
  if (!departure && current < Date.parse(`${run.service_date}T06:00:00Z`)) return { status: "planned", reason: "service-day-not-open", missingSince: null };
  if (arrival && current > arrival + 180 * 60_000) return { status: "unknown_completion", reason: "arrival-window-passed-without-fact", missingSince: new Date(arrival + 60 * 60_000).toISOString() };
  if (Number.isFinite(serviceEnd) && current > serviceEnd + 6 * 3600_000) return { status: "unknown_completion", reason: "service-day-ended-without-fact", missingSince: new Date(serviceEnd).toISOString() };
  return { status: "unobserved", reason: "expected-run-without-current-fact", missingSince: departure ? new Date(departure + 20 * 60_000).toISOString() : `${run.service_date}T06:00:00.000Z` };
}
...
export async function refreshExpectedRunCoverage(env, now = new Date().toISOString()) {
  const expected = rows(await env.DB.prepare(`SELECT x.*,(SELECT COUNT(*) FROM events e WHERE e.run_id=x.run_id) live_event_count,(SELECT MAX(occurred_at) FROM events e WHERE e.run_id=x.run_id) live_last_observation,(SELECT station FROM events e WHERE e.run_id=x.run_id AND station IS NOT NULL ORDER BY occurred_at DESC LIMIT 1) live_last_station FROM expected_train_runs x WHERE service_date>=date('now','-1 day') AND service_date<=date('now','+1 day') ORDER BY service_date,train_number`).all());
  const statements=[];let silent=0,active=0,planned=0;
  for(const run of expected){run.observation_count=Number(run.live_event_count)||0;run.last_observation_at=run.live_last_observation||run.last_observation_at;run.last_station=run.live_last_station||run.last_station;const state=classifyExpectedRun(run,now);if(state.status==="active")active+=1;else if(state.status==="planned")planned+=1;else silent+=1;
    ...
    statements.push(env.DB.prepare(`INSERT INTO ops_movements(movement_id,run_id,train_number,movement_type,origin,destination,route,status,last_station,last_observed_at,confidence,position_status,metadata_json) VALUES(?1,?2,?3,'passenger',?4,?5,?6,?7,?8,?9,?10,?11,?12) ON CONFLICT(run_id) DO UPDATE SET status=excluded.status,last_station=excluded.last_station,last_observed_at=excluded.last_observed_at,confidence=excluded.confidence,position_status=excluded.position_status,metadata_json=excluded.metadata_json WHERE ops_movements.status IN ('planned','unobserved','unknown_completion')`).bind(run.run_id,run.run_id,run.train_number,run.origin,run.destination,run.route,state.status,run.last_station,run.last_observation_at||run.scheduled_departure||now,state.status==="active"?.7:state.status==="planned"?.35:.2,state.status==="active"?"reported":state.status,JSON.stringify({expectedOnly:run.observation_count===0,expectedId:run.expected_id,statusReason:state.reason,scheduledDeparture:run.scheduled_departure,scheduledArrival:run.scheduled_arrival})));
  ...
  for(let index=0;index<statements.length;index+=75)await env.DB.batch(statements.slice(index,index+75));return {total:expected.length,silent,active,planned};
}
