...
export function buildTwinHypotheses({ event, candidates = [], now = new Date().toISOString(), routeHint = "", maximum = 3, previousState = null }) {
  ...
  const hypotheses = scored.map((item, index) => {
    ...
    const progress = held?0:clamp(ageMinutes / item.p50), geometry = railGeometryCoordinates(item.edge.geometry_json), point = positionStatus === "unknown" ? null : interpolateRailGeometry(geometry, progress);
...
export async function runIntelligenceCycle(env, now = new Date().toISOString()) {
  ...
  try {
    ...
    const previousStates=new Map(rows(await env.DB.prepare("SELECT run_id,operational_state,state_since,anchor_node_id,next_node_id,previous_node_id FROM twin_states").all()).map((state)=>[state.run_id,state])),predictionStatements=[],predictionByRun=new Map();
