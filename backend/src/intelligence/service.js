...
export async function runIntelligenceCycle(env, now = new Date().toISOString()) {
  ...
  try {
    ...
    for(const event of latestByRun.values()){
      ...
      const twin=buildTwinHypotheses({event,candidates,now,routeHint:[event.route,event.origin,event.destination].filter(Boolean).join(" "),previousState:previousStates.get(event.run_id)});if(!twin.state)continue;
