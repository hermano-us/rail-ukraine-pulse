...
import { applyCalibrationV4, applyCalibrationV5, refreshCalibrationProfilesV4 } from "./calibration-v4.js";
...
export async function runIntelligenceCycle(env, now = new Date().toISOString()) {
  ...
  const counters = { nodes: 0, edges: 0, observations: 0, predictions: 0, resolved: 0, replayed: 0, anomalies: 0, routesCalculated: 0, linksCreated: 0, calibrationProfiles: 0, stateTransitions: 0, prospectiveEvaluations: 0, expectedRuns: 0, silentRuns: 0, fusedObservations: 0, fusionAmbiguous: 0, boardPriorities: 0, twinsRecalculated: 0, activeCollectors: 0, reliabilityUrgentStations: 0 };
  ...
  try {
    ...
    const calibration=await refreshCalibrationProfiles(env,now,stationAliases),calibrationV3=await refreshCalibrationProfilesV3(env,now,stationAliases),calibrationV4=await refreshCalibrationProfilesV4(env,now,stationAliases);counters.calibrationProfiles=calibration.updated+calibrationV3.updated+calibrationV4.updated;
