...
import { fuseRecentObservationsV3 } from "./observation-fusion-v3.js";
...
export async function runIntelligenceCycle(env, now = new Date().toISOString()) {
  ...
  const counters = { nodes: 0, edges: 0, observations: 0, predictions: 0, resolved: 0, replayed: 0, anomalies: 0, routesCalculated: 0, linksCreated: 0, calibrationProfiles: 0, stateTransitions: 0, prospectiveEvaluations: 0, expectedRuns: 0, silentRuns: 0, fusedObservations: 0, fusionAmbiguous: 0, boardPriorities: 0, twinsRecalculated: 0 };
  ...
  try {
    ...
    const fusionResult=await fuseRecentObservationsV3(env,now,stationAliases); counters.fusedObservations=fusionResult.groups; counters.fusionAmbiguous=fusionResult.ambiguous;
    ...
    const boardCoverage=await refreshStationCoveragePriorities(env,now,stationAliases);counters.boardPriorities=boardCoverage.total;
    ...
    await env.DB.prepare("UPDATE intelligence_cycles SET fusion_ambiguous=?1,board_priorities=?2,twins_recalculated=?3 WHERE cycle_id=?4").bind(counters.fusionAmbiguous,counters.boardPriorities,counters.twinsRecalculated,cycleId).run();
