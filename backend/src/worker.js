...
async function handleCollectorBoardPriorities(request,env){
  ...
  const [result,collectorsResult]=await Promise.all([
    env.DB.prepare("SELECT p.station_id,p.station_name,p.priority_score,p.expected_runs,p.silent_runs,p.ambiguous_twins,p.overdue_twins,p.minutes_since_fact,p.reason_json,p.calculated_at,p.priority_tier,p.collector_failures,p.next_eligible_at,COALESCE(c.target_interval_minutes,20) target_interval_minutes,COALESCE(c.request_weight,1) request_weight FROM station_coverage_priorities p LEFT JOIN station_collection_plan c ON c.station_id=p.station_id ORDER BY CASE p.priority_tier WHEN 'critical' THEN 0 WHEN 'corridor' THEN 1 ELSE 2 END,COALESCE(c.target_interval_minutes,20),p.priority_score DESC LIMIT 200").all(),
  ...
  const stations=(result?.results||[]).map((item)=>{let reasons=[];try{reasons=JSON.parse(item.reason_json||"[]");}catch{}return {stationId:item.station_id,stationName:item.station_name,priorityScore:Number(item.priority_score)||0,priorityTier:item.priority_tier||"background",targetIntervalMinutes:Number(item.target_interval_minutes)||20,requestWeight:Number(item.request_weight)||1,expectedRuns:Number(item.expected_runs)||0,silentRuns:Number(item.silent_runs)||0,ambiguousTwins:Number(item.ambiguous_twins)||0,overdueTwins:Number(item.overdue_twins)||0,minutesSinceFact:item.minutes_since_fact==null?null:Number(item.minutes_since_fact),collectorFailures:Number(item.collector_failures)||0,nextEligibleAt:item.next_eligible_at||null,reasons,calculatedAt:item.calculated_at};});
  ...
  return json({generatedAt:new Date().toISOString(),strategy:"information-gain-v6-adaptive-coverage",recommendedRequestBudget,fleet:{activeCollectors,degradedCollectors},stations},{headers:{"Cache-Control":"no-store"}},request,env);
}
