...
export async function handleFreightRequest(request, env, auth) {
  ...
  if (request.method === "POST" && path === "/api/v1/freight/ingest") {
    ...
    for (const item of observations) {
      ...
      statements.push(env.DB.prepare("INSERT INTO restricted_evidence(evidence_id,domain,source_id,source_url,occurred_at,received_at,content_fingerprint,evidence_excerpt,classification_json,corridor_code,confidence,sensitivity_level,review_status,created_at,updated_at) VALUES(?1,'rail_freight',?2,?3,?4,?5,?6,?7,?8,?9,?10,'restricted','pending',?5,?5) ON CONFLICT(evidence_id) DO UPDATE SET received_at=excluded.received_at,confidence=MAX(restricted_evidence.confidence,excluded.confidence),updated_at=excluded.updated_at").bind(String(item.observationId).slice(0, 180), String(item.sourceId).slice(0, 100), String(item.sourceUrl).slice(0, 500), occurredAt, now, String(item.contentFingerprint || "").slice(0, 80), String(item.evidenceExcerpt || "").slice(0, 400), JSON.stringify({ freightType: item.freightType, ingestion: "telegram-preview" }), String(item.corridor || "unresolved").slice(0, 80), confidence));
    }
