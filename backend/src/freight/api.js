...
export async function handleFreightRequest(request, env, auth) {
  ...
  if (request.method === "POST" && path === "/api/v1/freight/ingest") {
    ...
    const body = await request.json(); const observations = Array.isArray(body.observations) ? body.observations : []; const safetyQuarantine = Array.isArray(body.safetyQuarantine) ? body.safetyQuarantine : []; const sources = Array.isArray(body.sources) ? body.sources : [];
    if (observations.length > 500 || safetyQuarantine.length > 500 || sources.length > 50) return json({ error: "payload_too_large" }, 400);
    const now = new Date().toISOString(); const statements = []; let accepted = 0; let quarantined = 0;
    const candidateIds = [...new Set([
      ...observations.filter((item) => item?.observationId && String(item.sourceId || "").startsWith("freight-tg-")).map((item) => String(item.observationId).slice(0, 180)),
      ...safetyQuarantine.filter((item) => item?.quarantineId && String(item.sourceId || "").startsWith("freight-tg-")).map((item) => String(item.quarantineId).slice(0, 180)),
    ])];
    ...
    for (const item of safetyQuarantine) {
      if (!item.quarantineId || !String(item.sourceId || "").startsWith("freight-tg-") || !item.sourceUrl || item.publicEligible === true || "latitude" in item || "longitude" in item) continue;
      const evidenceId = String(item.quarantineId).slice(0, 180); if (seenIds.has(evidenceId)) continue; seenIds.add(evidenceId);
      const occurredAt = Number.isFinite(Date.parse(item.occurredAt)) ? new Date(item.occurredAt).toISOString() : null;
      if (!occurredAt || String(item.reason || "") !== "sensitive_content") continue;
      statements.push(env.DB.prepare("INSERT OR IGNORE INTO restricted_evidence(evidence_id,domain,source_id,source_url,occurred_at,received_at,content_fingerprint,evidence_excerpt,classification_json,corridor_code,confidence,sensitivity_level,review_status,created_at,updated_at) VALUES(?1,'rail_freight_safety',?2,?3,?4,?5,?6,'[redacted: safety review required]',?7,NULL,0,'highly_restricted','pending',?5,?5)").bind(evidenceId, String(item.sourceId).slice(0, 100), String(item.sourceUrl).slice(0, 500), occurredAt, now, String(item.contentFingerprint || "").slice(0, 80), JSON.stringify({ ingestion: "safety-quarantine-v1", reason: "sensitive_content", rawStored: false, mappingAllowed: false, entityExtractionAllowed: false }))); quarantined += 1;
    }
    ...
    return json({ ok: true, received: observations.length, accepted, quarantined, publicObjects: 0, policy: "private-moderation-with-isolated-safety-quarantine" }, 202);
  }
  if (request.method === "GET" && path === "/api/admin/freight") {
    ...
    return json({ sources: rows(health), moderation: rows(counts), recent: rows(recent), publicObjects: 0, policy: { exactPositions: false, militaryContentStored: false, safetyQuarantine: true, minimumIndependentConfirmations: 2 } });
  }
  return null;
}
