...
export function buildRailGraph({ osmElements = [], registry, maximumSegmentKm = 250, maximumNeighbors = 12 } = {}) {
  const nodeById=new Map(osmElements.filter((item)=>item.type==="node"&&insideBounds(Number(item.lat),Number(item.lon))).map((item)=>[String(item.id),[Number(item.lon),Number(item.lat)]]));
  const routeRelations=osmElements.filter((item)=>item.type==="relation"&&item.tags?.type==="route"&&["train","railway","tracks"].includes(item.tags?.route));
  const relationsByWay=new Map();
  for(const relation of routeRelations)for(const member of relation.members||[])if(member.type==="way"){
    const key=String(member.ref),group=relationsByWay.get(key)||[];group.push({id:String(relation.id),route:relation.tags.route,ref:relation.tags.ref||null,name:relation.tags.name||null,operator:relation.tags.operator||null,network:relation.tags.network||null});relationsByWay.set(key,group);
  }
  ...
  const addEdge=(from,to,way)=>{const fromPoint=nodeById.get(from),toPoint=nodeById.get(to);if(!fromPoint||!toPoint)return;const distance=haversineKm(fromPoint,toPoint);if(!(distance>0&&distance<25))return;const service=way.tags.service||null,usage=way.tags.usage||null,status=way.tags.status||null;let penalty=1;if(service==="yard")penalty=8;else if(service==="siding")penalty=5;else if(service==="spur")penalty=4;else if(service==="crossover")penalty=2;if(usage==="industrial")penalty*=2.5;else if(usage==="branch")penalty*=1.2;const routeRelationsForWay=relationsByWay.get(String(way.id))||[];if(routeRelationsForWay.length)penalty*=.78;const group=adjacency.get(from)||[];group.push({to,distance,cost:distance*penalty,wayId:String(way.id),railwayType:way.tags.railway,usage,service,status,tracks:Number(way.tags.tracks)||null,electrified:way.tags.electrified||null,gauge:way.tags.gauge||null,maxspeed:Number(way.tags.maxspeed)||null,routeRelations:routeRelationsForWay});adjacency.set(from,group);railNodeIds.add(from);railNodeIds.add(to);};
  ...
  for(const station of registry.stations){const start=station.graphNodeId;if(!start||!adjacency.has(start))continue;const heap=new MinHeap(),costByNode=new Map([[start,0]]),distanceByNode=new Map([[start,0]]),previous=new Map();heap.push({nodeId:start,distance:0});let found=0;
    while(heap.size&&found<maximumNeighbors){const current=heap.pop();if(current.distance!==costByNode.get(current.nodeId)||(distanceByNode.get(current.nodeId)||0)>maximumSegmentKm)continue;const destinations=(stationIdsAtNode.get(current.nodeId)||[]).filter((id)=>id!==station.stationId);if(destinations.length){for(const destinationId of destinations){const path=[],steps=[];let cursor=current.nodeId;while(cursor){path.push(cursor);const step=previous.get(cursor);if(!step)break;steps.push(step.edge);cursor=step.nodeId;}path.reverse();steps.reverse();const coordinates=simplifyGeometry(path.map((id)=>nodeById.get(id)).filter(Boolean));if(coordinates.length>=2){const wayIds=[...new Set(steps.map((edge)=>edge.wayId))],relationIds=[...new Set(steps.flatMap((edge)=>edge.routeRelations.map((relation)=>relation.id)))],services=[...new Set(steps.map((edge)=>edge.service).filter(Boolean))],serviceShare=steps.filter((edge)=>edge.service).length/Math.max(1,steps.length),usages=[...new Set(steps.map((edge)=>edge.usage).filter(Boolean))],gauges=[...new Set(steps.map((edge)=>edge.gauge).filter(Boolean))],electrification=[...new Set(steps.map((edge)=>edge.electrified).filter(Boolean))],key=`${station.stationId}>${destinationId}`,distanceKm=distanceByNode.get(current.nodeId),existing=segments.get(key);if(!existing||current.distance<existing.routingCost)segments.set(key,{segmentId:key,fromStationId:station.stationId,toStationId:destinationId,geometry:{type:"LineString",coordinates},distanceKm:Number(distanceKm.toFixed(3)),routingCost:Number(current.distance.toFixed(3)),railwayType:steps.some((edge)=>edge.railwayType==="narrow_gauge")?"narrow_gauge":"rail",usageType:usages.length===1?usages[0]:null,serviceTypes:services,serviceShare:Number(serviceShare.toFixed(3)),trackCount:Math.max(0,...steps.map((edge)=>edge.tracks||0))||null,electrified:electrification.length===1?electrification[0]:null,gauges,sourceWayIds:wayIds,routeRelationIds:relationIds,geometryQuality:Number(Math.max(.7,.99-serviceShare*.12).toFixed(3))});}}found+=destinations.length;continue;}
      for(const edge of adjacency.get(current.nodeId)||[]){const nextCost=current.distance+edge.cost,nextDistance=(distanceByNode.get(current.nodeId)||0)+edge.distance;if(nextCost>=Number(costByNode.get(edge.to)??Infinity)||nextDistance>maximumSegmentKm)continue;costByNode.set(edge.to,nextCost);distanceByNode.set(edge.to,nextDistance);previous.set(edge.to,{nodeId:current.nodeId,edge});heap.push({nodeId:edge.to,distance:nextCost});}
    }
  }
  const undirectedSegments=new Map();for(const segment of segments.values()){const pair=[segment.fromStationId,segment.toStationId].sort(),key=`${pair[0]}<->${pair[1]}`,existing=undirectedSegments.get(key);if(existing&&existing.routingCost<=segment.routingCost)continue;const canonicalDirection=segment.fromStationId===pair[0],geometry=canonicalDirection?segment.geometry:{...segment.geometry,coordinates:[...segment.geometry.coordinates].reverse()};undirectedSegments.set(key,{...segment,segmentId:key,fromStationId:pair[0],toStationId:pair[1],geometry,bidirectional:true});}
  return { stations:registry.stations,segments:[...undirectedSegments.values()],routeRelations,unmatchedStations,stats:{osmNodes:nodeById.size,railWays:ways.length,routeRelations:routeRelations.length,railNodes:railNodeIds.size,matchedStations:registry.stations.length-unmatchedStations.length,unmatchedStations:unmatchedStations.length,segments:undirectedSegments.size} };
}
