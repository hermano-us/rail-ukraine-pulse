...
const layer=L.layerGroup().addTo(map); let controller; let searchController; let currentStations=[]; let selectedMarker=null; let debounce; let searchDebounce;
...
function renderStationRows(stations){
  ...
  list.innerHTML=stations.length?stations.slice(0,150).map(s=>`<button class="station-row" data-id="${escape(s.id)}"><i class="status-pin ${escape(s.status)}"></i><span><strong>${escape(s.name)}</strong><small>${escape([s.city,s.address].filter(Boolean).join(" · ")||"Адрес не указан")}</small></span><em>${escape(statusLabels[s.status]||statusLabels.unknown)}</em></button>`).join(""):'<p class="empty">Ничего не найдено.</p>';
  list.querySelectorAll("[data-id]").forEach(button=>button.onclick=()=>{const station=stations.find(s=>s.id===button.dataset.id);if(station){map.setView([station.lat,station.lng],15);openStation(station.id);}});
}
function renderList(mode){
  const term=$("search").value.trim().toLowerCase();
  if(mode==="clusters"){if(!term)$("station-list").innerHTML='<p class="empty">Приблизьте карту: количество показано кластерами без перегрузки карты.</p>';return;}
  renderStationRows(currentStations.filter(s=>!term||[s.name,s.brand,s.city,s.address].some(v=>String(v||"").toLowerCase().includes(term))));
}
async function searchCatalog(){
  const term=$("search").value.trim();
  if(term.length<2){renderList(map.getZoom()<11?"clusters":"stations");return;}
  searchController?.abort();searchController=new AbortController();
  try{const data=await fuelFetch(`/api/fuel/v1/search?q=${encodeURIComponent(term)}&limit=75`,{signal:searchController.signal});renderStationRows(data.stations||[]);}
  catch(error){if(error.name!=="AbortError")$("station-list").innerHTML=`<p class="empty">Поиск временно недоступен: ${escape(error.message)}</p>`;}
...
function renderAccessibility(s){
  const a=s.accessibility;if(!a)return;
  const url=safeExternalUrl(a.sourceUrl),summary=a.summary||{};
  const card=`<section class="accessibility-card"><small>&#1041;&#1077;&#1079;&#1073;&#1072;&#1088;&#1100;&#1077;&#1088;&#1085;&#1086;&#1089;&#1090;&#1100; &#183; &#1086;&#1092;&#1080;&#1094;&#1080;&#1072;&#1083;&#1100;&#1085;&#1099;&#1077; &#1086;&#1090;&#1082;&#1088;&#1099;&#1090;&#1099;&#1077; &#1076;&#1072;&#1085;&#1085;&#1099;&#1077;</small><strong>${escape(a.assessment||"\u041e\u0446\u0435\u043d\u043a\u0430 \u043d\u0435 \u0443\u043a\u0430\u0437\u0430\u043d\u0430")}${Number.isFinite(Number(a.rating))?` &#183; ${Math.round(Number(a.rating)*100)}%`:""}</strong><div class="accessibility-stats"><span>&#1044;&#1072; <b>${Number(summary.yes||0)}</b></span><span>&#1053;&#1077;&#1090; <b>${Number(summary.no||0)}</b></span><span>&#1060;&#1086;&#1090;&#1086; <b>${Number(a.photoCount||0)}</b></span></div><p>&#1055;&#1088;&#1086;&#1074;&#1077;&#1088;&#1082;&#1072;: ${escape(a.assessedAt||"\u0434\u0430\u0442\u0430 \u043d\u0435 \u0443\u043a\u0430\u0437\u0430\u043d\u0430")} &#183; &#1091;&#1074;&#1077;&#1088;&#1077;&#1085;&#1085;&#1086;&#1089;&#1090;&#1100; &#1089;&#1086;&#1087;&#1086;&#1089;&#1090;&#1072;&#1074;&#1083;&#1077;&#1085;&#1080;&#1103; ${Math.round(Number(a.confidence||0)*100)}%</p>${url?`<a href="${escape(url)}" target="_blank" rel="noopener">&#1054;&#1090;&#1082;&#1088;&#1099;&#1090;&#1100; &#1082;&#1072;&#1088;&#1090;&#1086;&#1095;&#1082;&#1091; LUN</a>`:""}<footer>${escape(a.attribution||"CC BY 4.0")}</footer></section>`;
  $("details").querySelector(".detail-grid")?.insertAdjacentHTML("afterend",card);
}
...
async function openStation(id,marker){
  ...
  renderAccessibility(s); enrichDetails(s);
...
map.on("moveend",schedule);[$("status"),$("fuel")].forEach(element=>element.onchange=refresh);$("search").oninput=()=>{schedule();clearTimeout(searchDebounce);searchDebounce=setTimeout(searchCatalog,220)};$("locate").onclick=locate;$("nearby").onclick=locate;$("open-panel").onclick=()=>$("sidebar").classList.add("open");$("close-panel").onclick=()=>$("sidebar").classList.remove("open");$("theme").onclick=()=>document.body.classList.toggle("light");
checkHealth();refresh();
