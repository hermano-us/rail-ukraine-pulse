...
test("autonomous intelligence cycle persists graph, twin, operations and analytics", async () => {
  ...
  for(const file of ["0001_initial.sql","0004_model_observability.sql","0011_intelligence_platform.sql","0013_rail_intelligence_v2.sql","0014_rail_graph_registry.sql","0015_rail_intelligence_routing.sql","0016_rail_foundation_fusion.sql","0017_rail_intelligence_v3.sql","0018_observation_fusion_v2.sql","0020_observation_fusion_v3.sql","0021_operations_hub_v2.sql","0022_reliability_fusion_calibration_v4.sql","0024_rail_graph_v4_registry_quality.sql","0025_rail_intelligence_v6.sql","0026_route_aware_rail_graph.sql"]){
  ...
  assert.equal(database.prepare("SELECT COUNT(*) total FROM station_collection_plan").get().total>0,true);
  assert.equal(database.prepare("SELECT COUNT(*) total FROM twin_probability_history WHERE run_id='run-1'").get().total,1);
  assert.equal(database.prepare("SELECT status FROM model_releases WHERE model_version='rail-intelligence-v5'").get().status,"active");
  assert.equal(database.prepare("SELECT COUNT(*) total FROM model_quality_windows").get().total,1);
...
test("historical station pairs warm calibration while stale positions lose coordinates", async () => {
  const database=new DatabaseSync(":memory:");
  for(const file of ["0001_initial.sql","0004_model_observability.sql","0011_intelligence_platform.sql","0013_rail_intelligence_v2.sql","0014_rail_graph_registry.sql","0015_rail_intelligence_routing.sql","0016_rail_foundation_fusion.sql","0017_rail_intelligence_v3.sql","0018_observation_fusion_v2.sql","0020_observation_fusion_v3.sql","0021_operations_hub_v2.sql","0022_reliability_fusion_calibration_v4.sql","0024_rail_graph_v4_registry_quality.sql","0025_rail_intelligence_v6.sql","0026_route_aware_rail_graph.sql"]){database.exec(await readFile(new URL(`../backend/migrations/${file}`,import.meta.url),"utf8"));}
