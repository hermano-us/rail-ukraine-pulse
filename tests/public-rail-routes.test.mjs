...
test("train 779 public route follows imported OSM segments and scheduled waypoints",async()=>{
  ...
  assert.equal(result.routes[0].status,"ready");assert.equal(result.routes[0].versionId,versionId);assert.equal(result.routes[0].method,"itinerary-constrained-v1");assert.equal(result.routes[0].geometry.coordinates.length,7);assert.ok(result.routes[0].geometry.coordinates.some(([longitude,latitude])=>longitude===31.1&&latitude===50.72));assert.equal(result.calculated,1);
  database.close();
});
