...
test("restricted Operations Hub returns probabilistic freight corridors and station facts",async()=>{
  ...
  for(const file of ["0001_initial.sql","0010_secure_core.sql","0011_intelligence_platform.sql","0013_rail_intelligence_v2.sql","0014_rail_graph_registry.sql","0015_rail_intelligence_routing.sql","0016_rail_foundation_fusion.sql"])database.exec(await readFile(new URL(`../backend/migrations/${file}`,import.meta.url),"utf8"));
  ...
  const queueResponse=await handleIntelligencePlatformRequest(new Request("https://example.test/api/restricted/rail-intelligence"),{DB:new D1Adapter(database)},{id:"admin-1",permissions:["*"]},(value,status=200)=>new Response(JSON.stringify(value),{status,headers:{"Content-Type":"application/json"}}));const queueBody=await queueResponse.json();assert.equal(queueBody.entityResolution.pending[0].candidates[0].runId,"run-alt");
